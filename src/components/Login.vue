          ...
          <div class="form-box">
            <Form ref="formInline" :model="formDate" :rules="ruleInline">
              ...
              <FormItem prop="password">
                ...
                <i-button @click="sendPhoneCode" class="sendCodeBtn" type="success">发送验证码</i-button>
              </FormItem>
              <FormItem>
                  <Button type="error" size="large" @click="submitForm()" long>登录</Button>
              </FormItem>
          </Form>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    //发送验证码
    sendPhoneCode() {
      this.$axios.get('/api/pub/smscode/'+ this.formDate.username + '/1/').then(res=>{
        this.$Message.success('验证码已发送');
        this.formDate.password = '666666';
      })
    },
    //提交
    submitForm() {
      ...
      this.$axios.post('/api/users/register/',
        {'mobile':this.formDate.username,'sms_code':this.formDate.password})
        .then(res=>{
          if (res.data.id){
            self.$Message.success('登录成功');
            localStorage.setItem('token',res.data.token);
            localStorage.setItem('username',res.data.username);
            self.$store.commit('SET_USER_LOGIN_INFO',res.data);
            self.$router.push('/');
          }else {
